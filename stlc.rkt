#lang turnstile/lang
(provide (type-out → Nat) λ #%app #%datum (typed-out [[add1 : (→ Nat Nat)] S]) let define-type-alias)
...
#;(begin-for-syntax-
  ...
  (define type-identifier-table
    (make-free-id-table))
  (define value-identifier-table
    (make-free-id-table)))
#;(define-syntax define-type-alias
  (syntax-parser
    [(_ alias:id τ:any-type)
     #'(free-id-table-set! type-identifier-table alias τ)]))
(define-syntax define-type-alias
  (syntax-parser
    [(_ alias:id τ:any-type)
     #'(define-syntax- alias
